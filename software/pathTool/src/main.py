...
def collectPath(sub_folder):
    scripts = {}
    sub_dirs = sorted(os.listdir(sub_folder))
    for script_name in [f[:-3] for f in sorted(os.listdir(sub_folder)) if f.endswith('.py') and os.path.isfile(os.path.join(sub_folder, f))]:
        #module = __import__((script_name + '.py'))
        module = importlib.import_module(sub_folder + "\\" + script_name + ".py")
...
if __name__ == '__main__':
    ...
    paths = collectPath(sys.path[1] + '\\path')
